...
function shade() {
  ...
  var _loop = function _loop(i) {
    item_html = "<div class='itemlist'><li class='item'>{{num}}.{{name}}<div class='price'>{{price}}$</div><div class='del_btn' id={{id}}>X</div></li></div>";
    current_item_html = item_html.replace("{{num}}", i + 1).replace("{{name}}", list[i].name).replace("{{price}}", list[i].price).replace("{{id}}", i);
